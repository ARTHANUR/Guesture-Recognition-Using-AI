...
@handler
def closedRightFist(dots):
    if(bool(dots['rh']) and right_thumb_closed(dots) and right_fore_finger_down(dots) and right_middle_finger_down(dots) and right_ring_finger_down(dots) and right_small_finger_down(dots)):
        return True
    
@handler
def closedLeftFist(dots):
    if(bool(dots['lh']) and left_thumb_closed(dots) and left_fore_finger_down(dots) and left_middle_finger_down(dots) and left_ring_finger_down(dots) and left_small_finger_down(dots)):
        return True

@handler
def intersect(dots,g1,p1,g2,p2,close=20):
    return abs(dots[g1][str(p1)]['x']-dots[g2][str(p2)]['x'])<close and abs(dots[g1][str(p1)]['y']-dots[g2][str(p2)]['y'])<close
